...
public class ScatterBottomRightCorner implements ScatterBehaviour {
    ...
    private int[] cornerDirections = {2,2,1,1,1,2,2,3,3,3,3,3,3,3,0,0,1,1,0,0,1,1,1};
    ...
    public int[] scatter(GameView gv, int srcX, int srcY, int currentDirection) {
        ...
        if ((srcX % gv.getBlockSize() == 0) && (srcY % gv.getBlockSize() == 0)) {
            ...
            if(srcX  / gv.getBlockSize()  == 14 && srcY  / gv.getBlockSize() == 15 ){
                inCorner = true;
            }

            if(!inCorner) {
                AStar aStar = new AStar(gv, srcX / gv.getBlockSize(), srcY / gv.getBlockSize());
                List<Node> path = aStar.findPathTo(14, 15);
